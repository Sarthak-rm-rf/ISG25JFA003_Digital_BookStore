        ...
        @if (loading && bookReviews.length === 0) {
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
            @for (bookReview of bookReviews; track bookReview.bookId) {
              <div class="bg-card border border-border rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300">
                <!-- Book Header -->
                <div class="p-6 pb-0">
                  <div class="flex flex-col items-center text-center mb-6">
                    ...
                    @if (getBookDetails(bookReview.bookId)?.imageUrl) {
                      <img 
                        ...
                        class="w-48 h-72 object-cover rounded-lg shadow-md mb-4">
                    } @else {
                      <div class="w-48 h-72 bg-accent rounded-lg shadow-md flex items-center justify-center mb-4">
                        <span class="material-icons text-6xl text-[#ff5722]">menu_book</span>
                      </div>
                    }

                    <!-- Book Details -->
                    <div class="space-y-2">
                      <h2 class="text-2xl font-bold text-foreground">{{ bookReview.bookTitle }}</h2>
                      @if (getBookDetails(bookReview.bookId)) {
                        <div class="text-muted-foreground">
                          <p class="text-lg">By {{ getBookDetails(bookReview.bookId)?.author?.name || 'Unknown Author' }}</p>
                          <p>{{ getBookDetails(bookReview.bookId)?.category?.name || 'Uncategorized' }}</p>
                        </div>
                      }
                      <div class="flex items-center justify-center gap-3 mt-2">
                        <div class="flex items-center bg-accent/10 px-3 py-1 rounded-full">
                          <span class="material-icons text-[#ff5722] mr-1">star</span>
                          <span class="text-foreground font-medium">{{ getAverageRating(bookReview.reviews) }}/5</span>
                        </div>
                        <span class="text-muted-foreground">({{ bookReview.reviews.length }} reviews)</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <!-- Reviews Section -->
                <div class="border-t border-border bg-accent/5">
                  <div class="p-6">
                    <div class="flex items-center justify-between mb-6">
                      <h3 class="text-lg font-semibold text-foreground">Reviews</h3>
                      <span class="text-sm text-muted-foreground bg-accent/10 px-3 py-1 rounded-full">
                        {{ bookReview.reviews.length }} total
                      </span>
                    </div>
