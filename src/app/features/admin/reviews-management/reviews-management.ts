...
@Component({
  ...
  standalone: true,
  // Imports are preserved from your original file
  imports: [
    CommonModule,
    FormsModule,
      ...
      <div class="max-w-7xl mx-auto px-4">
        ...
        <div class="flex justify-between items-center mb-8">
          <div>
            ...
            <p class="text-gray-600">Manage book reviews and ratings</p>
          </div>
        </div>
        ...
        <div class="text-center py-16">
          ...
          ></div>
          <p class="mt-4 text-gray-600 text-lg">Loading reviews...</p>
        </div>
        } @else {
        <div class="space-y-8">
          ...
          >
            <div class="p-4 border-b border-gray-200 bg-gray-50">
              <div class="flex items-start gap-4">
                ...
                <div
                  class="w-20 h-28 bg-[#fff3e0] rounded-lg shadow-md flex items-center justify-center"
                >
                  <span class="material-icons text-2xl text-[#ff5722]"
                    >menu_book</span
                  >
                </div>
                }
                ...
                <div class="flex-1">
                  ...
                  <p class="text-sm text-gray-600 mb-2">
                    By
                    {{
                      getBookDetails(bookReview.bookId)?.author?.name ||
                        'Unknown Author'
                    }}
                    ·
                    {{
                      getBookDetails(bookReview.bookId)?.category?.name ||
                        'Uncategorized'
                    }}
                  </p>
                  }
                  <div class="flex items-center gap-3">
                    <div
                      class="flex items-center bg-[#fff3e0] px-3 py-1 rounded-full"
                    >
                      <span class="material-icons text-sm text-[#ff5722] mr-1"
                        >star</span
                      >
                      <span class="text-sm font-medium"
                        >{{ getAverageRating(bookReview.reviews) }}/5</span
                      >
                    </div>
                    <span class="text-sm text-gray-600"
                      >{{ bookReview.reviews.length }} reviews</span
                    >
                  </div>
                </div>
              </div>
                  ...
                  <div class="flex justify-between items-start gap-4">
                    <div class="flex-1">
                      <div class="flex items-center gap-3 mb-2">
                        <div
                          class="flex bg-[#fff3e0] px-2 py-1 rounded-full"
                        >
                    ...
                    <button
                      ...
                      class="text-red-600 hover:text-red-800 transition-colors flex items-center p-2 rounded-full hover:bg-red-50"
                    >
          ...
          <div
            class="col-span-2 text-center py-16 bg-white border border-gray-200 rounded-xl"
          >
            <span class="material-icons text-6xl text-[#ff5722] mb-4"
              >rate_review</span
            >
...
export class ReviewsManagementComponent implements OnInit {
  // All TypeScript logic is 100% from your original file
  ...
  async loadReviews(): Promise<void> {
    try {
      ...
      const reviewPromises = books
        .filter(
          (book): book is Book & { bookId: number } => book.bookId !== undefined
        )
        .map((book) =>
          this.reviewService
            .getBookReviews(book.bookId)
            .toPromise()
            .then((reviews) => ({
              bookId: book.bookId,
  ...
  async confirmDeleteReview(): Promise<void> {
    ...
    try {
      ...
      await this.reviewService
        .deleteReviewByAdmin(this.reviewToDelete.reviewId)
        .toPromise();
