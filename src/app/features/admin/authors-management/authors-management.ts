          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @for (author of authors; track author.authorId) {
              <div class="bg-white border border-gray-200 p-6 rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
                ...
                <div class="mb-6">
                  <p class="text-gray-600 text-sm leading-relaxed">{{ author.biography || 'No biography available for this author.' }}</p>
                </div>
        ...
        <div class="fixed inset-0 bg-black bg-opacity-30 overflow-y-auto h-full w-full z-50 flex items-center justify-center p-4">
          <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md transform transition-all">
            <div class="p-6">
              ...
              <div class="mb-8">
                <label for="newAuthorBiography" class="block text-sm font-semibold text-gray-700 mb-2">
                  Biography
                </label>
                <textarea
  ...
  confirmAddAuthor(): void {
    if (this.newAuthorName.trim()) {
      ...
      this.authorService.addAuthor(newAuthor).subscribe({
        next: () => {
          this.successMessage = `Author "${newAuthor.name}" added successfully`;
          this.loadAuthors();
        ...
        error: (err) => {
          ...
          this.errorMessage = 'Error adding author: ' + (err.message || 'Unknown error');
          this.showAddModal = false;
        }
      });
    }
  ...
  confirmEditAuthor(): void {
    if (this.authorToEdit && this.editAuthorName.trim()) {
      ...
      this.authorService.updateAuthor(this.authorToEdit.authorId!, updatedAuthor).subscribe({
        next: () => {
          this.successMessage = `Author "${updatedAuthor.name}" updated successfully`;
          this.loadAuthors();
        ...
        error: (err) => {
          ...
          this.errorMessage = 'Error updating author: ' + (err.message || 'Unknown error');
          this.showEditModal = false;
          this.authorToEdit = null;
        }
      });
  ...
  confirmDeleteAuthor(): void {
    if (this.authorToDelete && this.authorToDelete.authorId) {
      this.authorService.deleteAuthor(this.authorToDelete.authorId).subscribe({
        next: () => {
          this.successMessage = `Author "${this.authorToDelete?.name}" deleted successfully`;
          this.loadAuthors();
        ...
        error: (err) => {
          ...
          this.errorMessage = 'Error deleting author: ' + (err.message || 'Unknown error');
          this.showDeleteModal = false;
          this.authorToDelete = null;
        }
      });
